...
const NewsCard = ({item, index}) => {
  ...
  const images = item?.files
    ?.filter(file => file?.mimeType?.split('/')[0] === 'image')
    ?.map(file => ({uri: Urls.BASE_URI + file?.filePath}));
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.contentContainer}>
        <Text numberOfLines={7} style={styles.content}>
          {item?.content}
        </Text>
      </View>
      {/* <AutoHeightWebView
        ...
        <ScrollView horizontal={true}>
          {item?.files?.map(item => (
            <FileContent
              item={item}
              key={item._id}
              onPressImage={() => setIsVisible(true)}
            />
          ))}
        </ScrollView>
      </View>
